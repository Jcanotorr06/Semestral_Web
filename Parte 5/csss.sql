...
-- Dumping database structure for csss
CREATE DATABASE IF NOT EXISTS `csss` /*!40100 DEFAULT CHARACTER SET utf8mb4 */;
USE `csss`;
...
DELIMITER //
CREATE DEFINER=`admin`@`%` PROCEDURE `agregar_medico`(
...
BEGIN
	...
	INSERT INTO medicos(id_usuario, id_especialidad, id_clinica)
	VALUES((SELECT usu.id FROM usuarios as usu WHERE usu.cedula = cedula_in), id_especialidad_in, id_clinica_in);
END//
DELIMITER ;

-- Dumping structure for procedure csss.check_datos_usuario_existen
DELIMITER //
CREATE DEFINER=`admin`@`%` PROCEDURE `check_datos_usuario_existen`(
	IN `cedula_in` VARCHAR(15),
	IN `email_in` VARCHAR(255)
)
BEGIN
...
CREATE TABLE IF NOT EXISTS `citas` (
  ...
  CONSTRAINT `citas_ibfk_1` FOREIGN KEY (`id_usuario`) REFERENCES `usuarios` (`id`),
  CONSTRAINT `citas_ibfk_2` FOREIGN KEY (`id_medico`) REFERENCES `medicos` (`id_usuario`),
  CONSTRAINT `citas_ibfk_3` FOREIGN KEY (`id_estado`) REFERENCES `estados` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- Dumping data for table csss.citas: ~0 rows (approximately)
...
-- Dumping data for table csss.dias_libres: ~0 rows (approximately)
...
-- Dumping data for table csss.estados: ~3 rows (approximately)
...
-- Dumping data for table csss.horario_medicos: ~3 rows (approximately)
/*!40000 ALTER TABLE `horario_medicos` DISABLE KEYS */;
INSERT INTO `horario_medicos` (`id_medico`, `id_dia`, `hora_entrada`, `hora_descanso`, `hora_regreso`, `hora_salida`) VALUES
	(3, 2, '04:00:00', '12:00:00', '12:30:00', '16:00:00'),
	(3, 1, '04:00:00', '12:00:00', '12:30:00', '16:00:00'),
	(3, 3, '04:00:00', '12:00:00', '12:30:00', '16:00:00');
...
-- Dumping data for table csss.medicos: ~1 rows (approximately)
/*!40000 ALTER TABLE `medicos` DISABLE KEYS */;
INSERT INTO `medicos` (`id_usuario`, `id_especialidad`, `id_clinica`) VALUES
	(3, 8, 5);
...
-- Dumping data for table csss.recuperar_contraseña: ~1 rows (approximately)
...
-- Dumping data for table csss.roles: ~3 rows (approximately)
...
-- Dumping data for table csss.usuarios: ~4 rows (approximately)
...
-- Dumping structure for procedure csss.ver_citas_medico_sesion
DELIMITER //
CREATE DEFINER=`admin`@`%` PROCEDURE `ver_citas_medico_sesion`(
	IN `id_usuario_sesion` INT
)
BEGIN
	SELECT cit.id, cit.nombre_paciente + ' ' + cit.apellido_paciente as nombre_paciente, cit.cedula_paciente, usu.nombre + ' ' + usu.apellido as nombre_medico, esp.especialidad, cit.fecha, cit.hora, pro.provincia, dis.distrito, cor.corregimiento, cli.clinica
	FROM citas AS cit
	...
	JOIN usuarios AS usu ON usu.id = med.id_usuario
...
-- Dumping structure for procedure csss.ver_citas_paciente_sesion
DELIMITER //
CREATE DEFINER=`admin`@`%` PROCEDURE `ver_citas_paciente_sesion`(
	IN `id_usuario_sesion` INT
)
BEGIN
	SELECT cit.id, cit.nombre_paciente + ' ' + cit.apellido_paciente as nombre_paciente, cit.cedula_paciente, usu.nombre + ' ' + usu.apellido as nombre_medico, esp.especialidad, cit.fecha, cit.hora, pro.provincia, dis.distrito, cor.corregimiento, cli.nombre as clinica
	FROM citas AS cit
	JOIN usuarios AS us ON us.id = cit.id_usuario
	JOIN medicos AS med ON med.id_usuario = cit.id_medico
	JOIN especialidades AS esp ON esp.id = med.id_especialidad
	...
	JOIN distritos AS dis ON dis.id = cor.id_distrito
	JOIN provincias AS pro ON pro.id = dis.id_provincia
	WHERE cit.id_usuario = id_usuario_sesion;
END//
DELIMITER ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`admin`@`%` VIEW `administrar_clinicas` AS SELECT cli.id, cli.clinica, cor.corregimiento, dis.distrito, pro.provincia
...
CREATE ALGORITHM=UNDEFINED DEFINER=`admin`@`%` VIEW `administrar_medicos` AS SELECT 
	usu.id, usu.nombre, usu.apellido, usu.cedula, usu.email, usu.contraseña, esp.especialidad, cli.clinica
...
CREATE ALGORITHM=UNDEFINED DEFINER=`admin`@`%` VIEW `listar_medicos` AS SELECT usu.nombre + ' ' + usu.apellido as nombre, usu.email, med.id_especialidad, med.id_clinica
FROM usuarios as usu
